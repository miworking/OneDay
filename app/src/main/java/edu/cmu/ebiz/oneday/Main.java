...
public class Main extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listView = (ListView) findViewById(R.id.onedaylistview);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (currentCountdownIndex != position) { //有其他的timer在运行
                    if (currentCountdownIndex == -1) {
                        bufferItem.setStatus(TodoItemBean.STOPPED);
                        buffertimetv.setBackgroundColor(Color.parseColor("#EEEEEE"));
                        buffertimetv.setTextColor(Color.parseColor("#000000"));

                    } else {
                        todoList.get(currentCountdownIndex).setStatus(TodoItemBean.STOPPED);
                    }
                    currentCountdownIndex = position;
                    todoList.get(position).setStatus(TodoItemBean.STARTED);
                } else {
